...
export function exportCSV(categories) {
  ...
  categories.forEach((cat) => {
    cat.items.forEach((it) =>
      rows.push([cat.name, it.name, it.checked ? "1" : "0"].join(","))
    );
  });
...
export function exportChecklist(checkedItemsSet, getAllItemsFn, getProgressFn) {
  ...
  const content = `CHECKLIST DE BOTIQUÍN DE PRIMEROS AUXILIOS - LIBRES Y VIAJEROS
...
export async function exportChecklistPDF(
  ...
  doc.setFontSize(18);
  doc.text(
    "CHECKLIST DE BOTIQUÍN DE PRIMEROS AUXILIOS - LIBRES Y VIAJEROS",
    margin,
    y
  );
  y += 24;
